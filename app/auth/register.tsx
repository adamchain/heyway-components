/*
 * REGISTER — Aligned to Home Liquid-Glass UI
 * - Same header, panel, inputs, button styles as Login/Home
 * - Fixed duplicate keys & conflicting style definitions
 */
...
export default function RegisterScreen() {
  ...
  const handlePhoneChange = (input: string) => {
    ...
    setPhoneValidation({ isValid: ok, error: !ok && e164Value.length > 3 ? 'Please enter a valid 10-digit phone number' : undefined });
  };

  const handleSMSRegister = async () => {
    if (!firstName.trim() || !phoneNumber.trim()) {
      Alert.alert('Error', 'Please enter your first name and phone number');
      return;
    }
    ...
    try {
      await sendSMSVerification({ phoneNumber: phoneNumber.trim(), firstName: firstName.trim(), isSignIn: false });
      setStep('verify');
    } catch (error) {
              ...
              {step === 'input' ? (
                <>
                  <View style={styles.inputContainer}>
                    <User size={18} color={HEYWAY_COLORS.text.secondary} />
                    <TextInput
                      style={styles.input}
                ...
                <>
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.9}
                  >
                    {isLoading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.primaryBtnText}>Verify & Create Account</Text>}
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  inputError: { borderColor: HEYWAY_COLORS.accent.error },
  errorText: { color: HEYWAY_COLORS.accent.error, fontSize: 12, marginBottom: 8, marginLeft: 4 },
